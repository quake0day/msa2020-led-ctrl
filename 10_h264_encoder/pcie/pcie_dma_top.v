...
module pcie_dma (
    ...
    // ---- H.264 编码器时钟 = iopll 干净 50MHz (clk_100mhz 已改配成 50MHz) ----
    // xk264 关键路径 ~13ns (CAVLC->bs_buf), 125MHz(8ns) 过不了; 50MHz(20ns) 富余。
    // 用 PLL 时钟(非 fabric 分频)保证时钟树平衡/hold 余量, 消除非确定性。
    // host BAR 仍在 pcie_clk, 经 axil_cdc 跨到 enc_clk。
    wire enc_clk = clk_100mhz;
    wire enc_rst;
    sync_reset #(.N(8)) sync_reset_enc_inst (.clk(enc_clk), .rst(rst_100mhz), .out(enc_rst));
...
pcie pcie_hip_inst (
    ...
    // ---- H.264 编码器外壳: 双时钟版 (host面 pcie_clk / 编码器面 enc_clk 50MHz)。
    //      内部双时钟真双口BRAM(输入pcie写/enc读, 输出enc写/pcie读, 时间上分离)
    //      + start toggle + busy/done 电平同步 + bytes/rinc done跨沿采, 取代 axil_cdc
    //      (多bit异步握手/未约束跨域=marginal-hold那类物理风险的根源)。 ----
    h264_wrap_dc #(
        .DATA_WIDTH(AXIL_DATA_WIDTH), .ADDR_WIDTH(AXIL_ADDR_WIDTH)
    ) axil_ram_inst (
        .pclk(pcie_clk), .prst(pcie_rst),
