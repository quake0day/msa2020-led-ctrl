...
不含 Corundum 全套 NIC（那是 [3.5](../3.5_Corundum_fork_on_MSA2020/)）。
...
- PCIe Gen3 x16 引脚：`TX BB34..AK42` / `RX AV30..AJ36` / refclk `AM34/AM33` /
  perst `AC26`（见顶层 README 或 3.5 的 PINDEF 提取）。
- PCIe 硬核 IP 生成方式见 3.5 的 `ip/pcie.tcl`（`chosen_devkit_hwtcl=NONE`，
  本板器件 `1SG280LN2F43E2VG`）。

## 现状

尚未开工。可从 3.5 的 PCIe 硬核 + 一个精简 DMA 数据通路起步，
