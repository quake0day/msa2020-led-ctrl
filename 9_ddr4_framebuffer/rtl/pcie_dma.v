...
//   (BAR0 读写 -> AXI-Lite)-> video_accel(视频计算加速器)。VID 0x1234 / DID 0x1002。
...
module pcie_dma (
    ...
    input  wire        usr_refclk0,   // 100MHz 板载用户时钟 (AD6, 1.8V) = clk100M(EMIF)
    ...
    output wire        ddr4_0_odt, ddr4_0_reset_n, ddr4_0_par,
    input  wire        ddr4_0_alert_n,
    inout  wire [63:0] ddr4_0_dq,
    inout  wire [7:0]  ddr4_0_dqs, ddr4_0_dqs_n, ddr4_0_dbi_n,
    // ---- DDR4 CH1 (丝印 DIMM1) ----
    ...
    output wire        ddr4_1_odt, ddr4_1_reset_n, ddr4_1_par,
    input  wire        ddr4_1_alert_n,
    inout  wire [63:0] ddr4_1_dq,
    inout  wire [7:0]  ddr4_1_dqs, ddr4_1_dqs_n, ddr4_1_dbi_n
);
    wire clk100M = usr_refclk0;   // EMIF/jtag_sys 系统时钟 = AD6
    ...
    // 注意: 只能有一个 Reset Release IP; 本设计用 jtag_sys 内部那个,
    // 由下方 jtag_sys 的 ninit_done_ninit_done 输出驱动 ninit_done。
...
pcie pcie_hip_inst (
    ...
    // =================================================================
    // DDR4 帧缓存后端: AXI-Lite(pcie_clk) -> 跨时钟桥 -> Avalon(clk100M) -> EMIF
    // =================================================================
    // I2C (SPD/retimer) 帧缓存功能不需要 -> 内部 tie-off (避免与 pcie_perstn 同 bank 电压冲突)
    wire sda_in = 1'b1, scl_in = 1'b1;   // 空闲高
    wire sda_oe, scl_oe;                 // jtag_sys 输出, 悬空不用

    // 桥 <-> jtag_sys.mem_* Avalon 信号 (clk100M 域)
    ...
    wire        cal_ok0, cal_fail0, usr_clk0;
    wire        cal_ok1, cal_fail1, usr_clk1;

    axil_ddr_bridge #(
        .DATA_WIDTH(AXIL_DATA_WIDTH), .ADDR_WIDTH(AXIL_ADDR_WIDTH), .AVMM_ADDR_WIDTH(36)
    ) axil_ram_inst (
        .clk(pcie_clk), .rst(pcie_rst),
        ...
        .mem_clk(clk100M),
        .avm_address(mb_address), .avm_read(mb_read), .avm_write(mb_write),
        .avm_writedata(mb_writedata), .avm_byteenable(mb_byteenable),
        .avm_readdata(mb_readdata), .avm_readdatavalid(mb_readdatavalid), .avm_waitrequest(mb_waitrequest),
        .cal_ok0(cal_ok0), .cal_ok1(cal_ok1)
    );

    // EMIF + DDR4 (复用 2_EMIF 的 jtag_sys; master_* 不用, mem_* 由桥驱动)
    jtag_sys u_jtag (
        .clk_clk               (clk100M),
        .reset_reset           (ninit_done),
        .ninit_done_ninit_done (ninit_done),
        .issp_sources_source   (),
        .issp_probes_probe     (32'd0),
        .mem_sources_source    (),
        .mem_probes_probe      (96'd0),
        ...
        .ddr0_ref_clk_clk      (ddr4_0_ref_clk), .ddr0_oct_oct_rzqin(ddr4_0_rzqin),
        .ddr0_mem_ck(ddr4_0_ck), .ddr0_mem_ck_n(ddr4_0_ck_n), .ddr0_mem_a(ddr4_0_a),
        .ddr0_mem_act_n(ddr4_0_act_n), .ddr0_mem_ba(ddr4_0_ba), .ddr0_mem_bg(ddr4_0_bg),
        .ddr0_mem_cke(ddr4_0_cke), .ddr0_mem_cs_n(ddr4_0_cs_n), .ddr0_mem_odt(ddr4_0_odt),
        .ddr0_mem_reset_n(ddr4_0_reset_n), .ddr0_mem_par(ddr4_0_par), .ddr0_mem_alert_n(ddr4_0_alert_n),
        .ddr0_mem_dqs(ddr4_0_dqs), .ddr0_mem_dqs_n(ddr4_0_dqs_n), .ddr0_mem_dq(ddr4_0_dq), .ddr0_mem_dbi_n(ddr4_0_dbi_n),
        .ddr0_status_local_cal_success(cal_ok0), .ddr0_status_local_cal_fail(cal_fail0), .ddr0_usrclk_clk(usr_clk0),
        .ddr1_ref_clk_clk      (ddr4_1_ref_clk), .ddr1_oct_oct_rzqin(ddr4_1_rzqin),
        .ddr1_mem_ck(ddr4_1_ck), .ddr1_mem_ck_n(ddr4_1_ck_n), .ddr1_mem_a(ddr4_1_a),
        .ddr1_mem_act_n(ddr4_1_act_n), .ddr1_mem_ba(ddr4_1_ba), .ddr1_mem_bg(ddr4_1_bg),
        .ddr1_mem_cke(ddr4_1_cke), .ddr1_mem_cs_n(ddr4_1_cs_n), .ddr1_mem_odt(ddr4_1_odt),
        .ddr1_mem_reset_n(ddr4_1_reset_n), .ddr1_mem_par(ddr4_1_par), .ddr1_mem_alert_n(ddr4_1_alert_n),
        .ddr1_mem_dqs(ddr4_1_dqs), .ddr1_mem_dqs_n(ddr4_1_dqs_n), .ddr1_mem_dq(ddr4_1_dq), .ddr1_mem_dbi_n(ddr4_1_dbi_n),
        .ddr1_status_local_cal_success(cal_ok1), .ddr1_status_local_cal_fail(cal_fail1), .ddr1_usrclk_clk(usr_clk1),
        .master_address(), .master_read(), .master_write(), .master_writedata(),
        .master_readdata(32'd0), .master_waitrequest(1'b0), .master_readdatavalid(1'b0),
        .master_byteenable(), .master_reset_reset()
    );
